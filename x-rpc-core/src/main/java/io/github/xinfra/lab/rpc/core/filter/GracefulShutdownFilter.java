...
import io.github.xinfra.lab.rpc.core.bootstrap.GracefulShutdown;
...
public class GracefulShutdownFilter implements Filter {
  ...
  public InvocationResult filter(Invoker invoker, Invocation invocation) {
    if (GracefulShutdown.INSTANCE.isShuttingDown()) {
      throw new RpcException("Server is shutting down, rejecting request");
    }
    GracefulShutdown.INSTANCE.incrementActive();
    try {
      return invoker.invoke(invocation);
    } finally {
      GracefulShutdown.INSTANCE.decrementActive();
    }
  }
}
